...
from minigeagent.models.test_models import (
    DeterministicModel,
...
def mock_prompts(side_effect):
    ...
    with patch("minigeagent.agents.utils.prompt_user.prompt_session.prompt", side_effect=se):
        with patch("minigeagent.agents.utils.prompt_user._multiline_prompt_session.prompt", side_effect=se):
            yield
...
def toolcall_config():
    ...
    config_path = Path("src/minigeagent/config/mini.yaml")
    with open(config_path) as f:
        config = yaml.safe_load(f)
    return config["agent"]
...
def test_help_command(model_factory):
    ...
    with mock_prompts(
        ...
        with patch("minigeagent.agents.interactive.console.print") as mock_print:
            agent = InteractiveAgent(
                model=factory(
                    [
    ...
    with patch.object(InteractiveAgent, "_stdin_is_interactive", return_value=True):
        with patch("builtins.input", side_effect=["10", "5.0"]):  # New step_limit=10, cost_limit=5.0
            with mock_prompts([""]):  # No new task
                with patch("minigeagent.agents.interactive.console.print"):  # Suppress console output
...
def test_limits_exceeded_multiple_times_with_continuation(model_factory):
    ...
    with patch.object(InteractiveAgent, "_stdin_is_interactive", return_value=True):
        with patch("builtins.input", side_effect=["2", "100.0", "10", "100.0"]):
            with mock_prompts([""]):  # No new task
                with patch("minigeagent.agents.interactive.console.print"):
