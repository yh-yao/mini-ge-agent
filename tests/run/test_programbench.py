...
from minigeagent.environments.docker import DockerEnvironment
from minigeagent.exceptions import Submitted
from minigeagent.run.benchmarks.programbench import copy_submission, main

# Lightweight image used for the real-docker tests. Already cached on machines
# that run mini-ge-agent's docker test suite (see tests/environments/test_docker.py).
...
def test_programbench_end_to_end_real_docker(real_programbench_first_instance, tmp_path, container_executable):
    ...
    with patch("minigeagent.run.benchmarks.programbench.get_model", side_effect=lambda **kw: _SubmittingModel()):
        main(
            slice_spec="",
...
def test_programbench_skip_existing_real_docker(real_programbench_first_instance, tmp_path, container_executable):
    ...
    with (
        patch("minigeagent.run.benchmarks.programbench.get_model") as mock_get_model,
        patch("minigeagent.run.benchmarks.programbench.get_environment") as mock_get_env,
    ):
        main(
            slice_spec="",
            filter_spec=f"^{iid}$",
