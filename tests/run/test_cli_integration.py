...
from minigeagent.run.mini import DEFAULT_CONFIG_FILE, app, main
...
def test_configure_if_first_time_called():
    ...
    with (
        patch("minigeagent.run.mini.configure_if_first_time") as mock_configure,
        patch("minigeagent.run.mini.get_agent") as mock_get_agent,
        patch("minigeagent.run.mini.get_model") as mock_get_model,
        patch("minigeagent.run.mini.get_environment") as mock_get_env,
        patch("minigeagent.run.mini.get_config_from_spec") as mock_get_config,
    ):
        # Setup mocks
        mock_model = Mock()
...
def test_mini_command_calls_run_interactive():
    ...
    with (
        patch("minigeagent.run.mini.configure_if_first_time"),
        patch("minigeagent.run.mini.get_agent") as mock_get_agent,
        patch("minigeagent.run.mini.get_model") as mock_get_model,
        patch("minigeagent.run.mini.get_environment") as mock_get_env,
        patch("minigeagent.run.mini.get_config_from_spec") as mock_get_config,
...
def test_mini_calls_prompt_when_no_task_provided():
    ...
    with (
        patch("minigeagent.run.mini.configure_if_first_time"),
        patch("minigeagent.run.mini._multiline_prompt") as mock_prompt,
        patch("minigeagent.run.mini.get_agent") as mock_get_agent,
        patch("minigeagent.run.mini.get_model") as mock_get_model,
        patch("minigeagent.run.mini.get_environment") as mock_get_env,
        patch("minigeagent.run.mini.get_config_from_spec") as mock_get_config,
...
def test_mini_help():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "minigeagent", "--help"],
        capture_output=True,
        text=True,
        timeout=10,
    )
    ...
    assert "Run mini-ge-agent in your local environment." in clean_output
...
def test_python_m_minigeagent_help():
    """Test that python -m minigeagent --help works correctly."""
    result = subprocess.run(
        [sys.executable, "-m", "minigeagent", "--help"],
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0
    assert "mini-ge-agent" in result.stdout
...
def test_mini_ge_agent_help():
    """Test that mini-ge-agent --help works correctly."""
    result = subprocess.run(
        ["mini-ge-agent", "--help"],
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0
    clean_output = strip_ansi_codes(result.stdout)
    assert "mini-ge-agent" in clean_output


def test_mini_ge_extra_help():
    """Test that mini-ge-extra --help works correctly."""
    result = subprocess.run(
        ["mini-ge-extra", "--help"],
        capture_output=True,
        text=True,
...
def test_mini_extra_subcommand_help(subcommand: str, aliases: list[str]):
    """Test that mini-ge-extra subcommands --help work correctly."""
    for alias in aliases:
        result = subprocess.run(
                ["mini-ge-extra", alias, "--help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
def test_mini_extra_config_help():
    """Test that mini-ge-extra config --help works correctly."""
    result = subprocess.run(
        ["mini-ge-extra", "config", "--help"],
        capture_output=True,
        text=True,
        timeout=10,
    )
...
def test_output_file_is_created(tmp_path):
    ...
    default_config_path = Path("src/minigeagent/config/default.yaml")
    ...
    with (
        ...
        patch("minigeagent.run.mini.get_model") as mock_get_model,
        patch("minigeagent.run.mini.get_environment") as mock_get_env,
        patch("minigeagent.agents.utils.prompt_user.prompt_session.prompt", return_value=""),
        patch("minigeagent.agents.utils.prompt_user._multiline_prompt_session.prompt", return_value=""),
    ):
        # Setup mocks
        mock_model = Mock()
        ...
        from minigeagent.exceptions import Submitted
