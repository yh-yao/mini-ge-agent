...
from minigeagent.run.utilities.config import app, configure_if_first_time, edit, set, setup, unset


@pytest.fixture(autouse=True)
def no_reload(monkeypatch):
    monkeypatch.setattr("minigeagent.run.utilities.config._reload_config", lambda: None)
...
class TestConfigSetup:
    ...
    def test_setup_with_all_inputs(self, tmp_path):
        ...
        with (
            patch("minigeagent.run.utilities.config.global_config_file", config_file),
            patch("minigeagent.run.utilities.config.prompt") as mock_prompt,
            patch("minigeagent.run.utilities.config.console.print"),
        ):
            ...
            assert "MGEA_MODEL_NAME='anthropic/claude-sonnet-4-5-20250929'" in content
            assert "ANTHROPIC_API_KEY='sk-test123'" in content
            assert "MGEA_CONFIGURED='true'" in content
    ...
    def test_setup_with_model_only(self, tmp_path):
        ...
        with (
            ...
            content = config_file.read_text()
            assert "MGEA_MODEL_NAME='gpt-4'" in content
            assert "MGEA_CONFIGURED='true'" in content
    ...
    def test_setup_with_empty_inputs(self, tmp_path):
        ...
        with (
            ...
            assert "MGEA_CONFIGURED='true'" in content
            assert "MGEA_MODEL_NAME" not in content
    ...
    def test_setup_with_existing_env_vars(self, tmp_path):
        ...
        with (
            ...
            patch("minigeagent.run.utilities.config.prompt") as mock_prompt,
            patch("minigeagent.run.utilities.config.console.print"),
            patch.dict(os.environ, {"MGEA_MODEL_NAME": "existing-model", "ANTHROPIC_API_KEY": "existing-key"}),
        ):
            ...
            content = config_file.read_text()
            assert "MGEA_MODEL_NAME='existing-model'" in content
    ...
    def test_setup_key_name_but_no_value(self, tmp_path):
        ...
        with (
            patch("minigeagent.run.utilities.config.global_config_file", config_file),
            patch("minigeagent.run.utilities.config.prompt") as mock_prompt,
            patch("minigeagent.run.utilities.config.console.print") as mock_print,
        ):
...
class TestConfigSet:
    ...
    def test_set_with_both_arguments_provided(self, tmp_path):
        ...
        with patch("minigeagent.run.utilities.config.global_config_file", config_file):
            set("MGEA_MODEL_NAME", "anthropic/claude-sonnet-4-5-20250929")
    ...
    def test_set_multiple_keys(self, tmp_path):
        ...
        with patch("minigeagent.run.utilities.config.global_config_file", config_file):
            set("MGEA_MODEL_NAME", "gpt-4")
            set("OPENAI_API_KEY", "sk-openai-test")
            set("MGEA_GLOBAL_COST_LIMIT", "10.00")

            content = config_file.read_text()
            assert "MGEA_MODEL_NAME='gpt-4'" in content
            assert "OPENAI_API_KEY='sk-openai-test'" in content
            assert "MGEA_GLOBAL_COST_LIMIT='10.00'" in content

    def test_set_overwrites_existing_key(self, tmp_path):
        """Test that setting a key overwrites existing value."""
        config_file = tmp_path / ".env"
        config_file.write_text("MGEA_MODEL_NAME=old-model\nOTHER_KEY=other-value\n")

        with patch("minigeagent.run.utilities.config.global_config_file", config_file):
            set("MGEA_MODEL_NAME", "new-model")

            content = config_file.read_text()
            assert "MGEA_MODEL_NAME='new-model'" in content
...
class TestConfigUnset:
    ...
    def test_unset_with_argument_provided(self, tmp_path):
        ...
        config_file = tmp_path / ".env"
        config_file.write_text("MGEA_MODEL_NAME='gpt-4'\nOPENAI_API_KEY='sk-test123'\n")

        with patch("minigeagent.run.utilities.config.global_config_file", config_file):
            unset("MGEA_MODEL_NAME")
    ...
    def test_unset_from_file_with_multiple_keys(self, tmp_path):
        ...
        config_file = tmp_path / ".env"
        config_file.write_text(
            "MGEA_MODEL_NAME='anthropic/claude-sonnet-4-5-20250929'\n"
            "ANTHROPIC_API_KEY='sk-anthropic-key'\n"
            "OPENAI_API_KEY='sk-openai-key'\n"
            "MGEA_CONFIGURED='true'\n"
        )
    ...
    def test_unset_configured_flag(self, tmp_path):
        ...
        with patch("minigeagent.run.utilities.config.global_config_file", config_file):
            unset("MGEA_CONFIGURED")

            content = config_file.read_text()
            # Configured flag should be removed
            assert "MGEA_CONFIGURED" not in content
            # Model should remain
            assert "MGEA_MODEL_NAME='gpt-4'" in content
...
class TestConfigEdit:
    ...
    def test_edit_with_default_editor(self, tmp_path):
        ...
        config_file = tmp_path / ".env"
        config_file.write_text("MGEA_MODEL_NAME=test")
...
class TestConfigureIfFirstTime:
    ...
    def test_configure_when_not_configured(self, tmp_path):
        """Test that setup is called when MGEA_CONFIGURED is not set."""
        config_file = tmp_path / ".env"

        with (
            patch("minigeagent.run.utilities.config.global_config_file", config_file),
            patch("minigeagent.run.utilities.config.setup") as mock_setup,
            patch("minigeagent.run.utilities.config.console.print") as mock_print,
            patch.dict(os.environ, {}, clear=True),  # Clear MGEA_CONFIGURED
        ):
    ...
    def test_skip_configure_when_already_configured(self, tmp_path):
        """Test that setup is not called when MGEA_CONFIGURED is set."""
        with (
            patch("minigeagent.run.utilities.config.setup") as mock_setup,
            patch.dict(os.environ, {"MGEA_CONFIGURED": "true"}),
        ):
            configure_if_first_time()
