...
from minigeagent.models import GlobalModelStats, get_model, get_model_class, get_model_name
...
class TestGetModelName:
    ...
    def test_input_model_name_takes_precedence(self):
        ...
        with patch.dict(os.environ, {"MGEA_MODEL_NAME": "env-model"}):
    ...
    def test_raises_error_when_no_model_configured(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(
                ValueError, match="No default model set. Please run `mini-ge-extra config setup` to set one."
            ):
                get_model_name(None, {})
...
class TestGetModelClass:
    ...
    def test_litellm_response_model_selection(self):
        ...
        from minigeagent.models.litellm_response_model import LitellmResponseModel
...
class TestGetModel:
    def test_config_deep_copy(self):
        ...
        with patch("minigeagent.models.get_model_class") as mock_get_class:
...
class TestGlobalModelStats:
    def test_prints_cost_limit_when_set(self, capsys):
        """Test that cost limit is printed when MGEA_GLOBAL_COST_LIMIT is set."""
        with patch.dict(os.environ, {"MGEA_GLOBAL_COST_LIMIT": "5.5"}, clear=True):
            GlobalModelStats()
            captured = capsys.readouterr()
            assert "Global cost/call limit: $5.5000 / 0" in captured.out

    def test_prints_call_limit_when_set(self, capsys):
        """Test that call limit is printed when MGEA_GLOBAL_CALL_LIMIT is set."""
        with patch.dict(os.environ, {"MGEA_GLOBAL_CALL_LIMIT": "10"}, clear=True):
            GlobalModelStats()
            captured = capsys.readouterr()
    ...
    def test_prints_both_limits_when_both_set(self, capsys):
        ...
        with patch.dict(os.environ, {"MGEA_GLOBAL_COST_LIMIT": "2.5", "MGEA_GLOBAL_CALL_LIMIT": "5"}, clear=True):
            GlobalModelStats()
            captured = capsys.readouterr()
            assert "Global cost/call limit: $2.5000 / 5" in captured.out

    def test_no_print_when_silent_startup_set(self, capsys):
        """Test that limits are not printed when MGEA_SILENT_STARTUP is set."""
        with patch.dict(
            os.environ,
            {"MGEA_GLOBAL_COST_LIMIT": "5.0", "MGEA_GLOBAL_CALL_LIMIT": "10", "MGEA_SILENT_STARTUP": "1"},
            clear=True,
        ):
            GlobalModelStats()
            captured = capsys.readouterr()
    ...
    def test_no_print_when_limits_are_zero(self, capsys):
        ...
        with patch.dict(os.environ, {"MGEA_GLOBAL_COST_LIMIT": "0", "MGEA_GLOBAL_CALL_LIMIT": "0"}, clear=True):
            GlobalModelStats()
            captured = capsys.readouterr()
            assert "Global cost/call limit" not in captured.out
