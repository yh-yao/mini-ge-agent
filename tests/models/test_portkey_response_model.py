...
from minigeagent.models import GLOBAL_MODEL_STATS
from minigeagent.models.portkey_response_model import PortkeyResponseAPIModel
from minigeagent.models.utils.actions_toolcall_response import BASH_TOOL_RESPONSE_API
...
def test_response_api_model_basic_query():
    ...
    with (
        patch("minigeagent.models.portkey_response_model.Portkey", mock_portkey_class),
        patch.dict(os.environ, {"PORTKEY_API_KEY": "test-key"}),
        patch("minigeagent.models.portkey_response_model.litellm.cost_calculator.completion_cost", return_value=0.01),
    ):
        mock_response = Mock()
...
def test_response_api_model_cost_tracking():
    ...
    with (
        patch("minigeagent.models.portkey_response_model.Portkey", mock_portkey_class),
        patch.dict(os.environ, {"PORTKEY_API_KEY": "test-key"}),
        patch("minigeagent.models.portkey_response_model.litellm.cost_calculator.completion_cost", return_value=0.05),
    ):
        mock_response = Mock()
...
def test_response_api_model_zero_cost_assertion():
    ...
    with (
        patch("minigeagent.models.portkey_response_model.Portkey", mock_portkey_class),
        patch.dict(os.environ, {"PORTKEY_API_KEY": "test-key"}),
        patch("minigeagent.models.portkey_response_model.litellm.cost_calculator.completion_cost", return_value=0.0),
    ):
        mock_response = Mock()
...
def test_response_api_model_retry_on_rate_limit():
    ...
    with (
        patch("minigeagent.models.portkey_response_model.Portkey", mock_portkey_class),
        patch.dict(os.environ, {"PORTKEY_API_KEY": "test-key", "MGEA_MODEL_RETRY_STOP_AFTER_ATTEMPT": "2"}),
        patch("minigeagent.models.portkey_response_model.litellm.cost_calculator.completion_cost", return_value=0.01),
    ):
        call_count = 0
