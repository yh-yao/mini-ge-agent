...
from minigeagent import global_config_file
...
You can edit it manually or use the [bold green]mini-ge-extra config set[/bold green] or [bold green]mini-ge-extra config edit[/bold green] commands.
...
def configure_if_first_time():
    if not os.getenv("MGEA_CONFIGURED"):
        console.print(Rule())
        setup()
        console.print(Rule())
...
def setup():
    ...
    default_model = prompt(
        ...
        default=os.getenv("MGEA_MODEL_NAME", ""),
    ).strip()
    if default_model:
        set_key(global_config_file, "MGEA_MODEL_NAME", default_model)
    console.print(
    ...
    set_key(global_config_file, "MGEA_CONFIGURED", "true")
    _reload_config()
    console.print(
        "\n[bold yellow]Config finished.[/bold yellow] If you want to revisit it, run [bold green]mini-ge-extra config setup[/bold green]."
    )
