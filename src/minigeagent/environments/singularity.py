...
class SingularityEnvironmentConfig(BaseModel):
    ...
    executable: str = os.getenv("MGEA_SINGULARITY_EXECUTABLE", "singularity")
...
class SingularityEnvironment:
    def __init__(
        ...
        self.logger = logger or logging.getLogger("minigeagent.environment")
    ...
    def _build_sandbox(self) -> Path:
        ...
        for attempt in range(max_retries):
            sandbox_dir = Path(tempfile.gettempdir()) / f"minigeagent-{uuid.uuid4().hex[:8]}"
            try:
                subprocess.run(
