...
from minigeagent.agents.default import AgentConfig, DefaultAgent
from minigeagent.agents.utils.prompt_user import _multiline_prompt, prompt_session
from minigeagent.exceptions import LimitsExceeded, Submitted, TimeExceeded, UserInterruption
from minigeagent.models.utils.content_string import get_content_string
...
class InteractiveAgent(DefaultAgent):
    ...
    def add_messages(self, *messages: dict) -> list[dict]:
        ...
        for msg in messages:
            ...
            if role == "assistant":
                console.print(
                    f"\n[red][bold]mini-ge-agent[/bold] (step [bold]{self.n_calls}[/bold], [bold]${self.cost:.2f}[/bold]):[/red]\n",
                    end="",
                    highlight=False,
                )
            else:
