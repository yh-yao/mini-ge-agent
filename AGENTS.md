# mini-ge-agent overview

- mini-ge-agent implements loop and graph engineering modes for GitHub issues and similar programming challenges.
- The project keeps each agent loop simple while making graph topology, gates, retries, and trajectories explicit.

The project is structured as

```bash
minigeagent/__init__  # Protocols/interfaces for all base classes
minigeagent/agents  # Agent control flow & loop
minigeagent/environments  # Executing agent actions
minigeagent/models  # LM interfaces
minigeagent/run  # Run scripts that serve as an entry point
```

- The project embraces polymorphism: Every individual class should be simple, but we offer alternatives
- The primary entry point is `mini --mode loop|graph`, which picks the corresponding configuration.
